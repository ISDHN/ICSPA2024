...
#ifndef CONFIG_FTRANCE_ONLY_ERRORTRACE
...
void traceback_display() {
	...
	Log("Traceback:");
	for (int i = 0; i < cnt; i++) {
		Log("%*sIn: %#x@%s", i * CONFIG_FTRANCE_PAD, "", call_stack[i], find_func_name(call_stack[i]));
	}
#endif
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)             \
	...
	INSTPAT("??????? ????? ????? 010 ????? 001 0011", slti, I, R(rd) = ((int32_t)src1 < imm) ? 1 : 0);
	...
	INSTPAT("??????? ????? ????? 000 ????? 110 0111", jalr, I, R(rd) = s->snpc; int dst = (src1 + imm) & ~1; s->dnpc = dst; log_jal_r(rd != 0, INSTPAT_INST(s) == 0x00008067, dst));
	// RV32I branch
	...
	INSTPAT("0011000 00010 00000 000 00000 111 0011", mret, N, { s->dnpc = MRW(mepc) + 4; mstatus_t *status = get_mstatus(); status->mie = status->mpie; status->mpie = 1; });
	// Misc
